...
function trx_addons_sc_fullheight_init(e, container) {
	...
	container.find('.sc_item_fullheight').each(function () {
		"use strict";
		...
		var wh = 0;
		var fullheight_row = jQuery(this).parents('.full-height-section');
		console.log(fullheight_row);
		...
		var ph = fullheight_item.height();
		var add = Math.ceil((wh - ph) / 2);
		fullheight_item.css({
		});
	});
}
				...
				TRX_ADDONS_STORAGE['swipers'][id] = new Swiper('.'+id, {
			        ...
			        autoplay: jQuery(this).hasClass('slider_noautoplay') 
								...
								: (jQuery(this).data('interval')=='' || isNaN(jQuery(this).data('interval')) 
									? 7000 
									: parseInt(jQuery(this).data('interval'),0)
									),
...
jQuery(document).on('action.init_hidden_elements', function(e, cont) {
	if (PJ_STORAGE['button_hover']) {
		jQuery('button:not(.search_submit):not([class*="sc_button_hover_"]),\
				...
				.sc_tabs ul.sc_tabs_titles li a:not([class*="sc_button_hover_"]),\
				.sc_item_button > a:not([class*="sc_button_hover_"]),\
				...
				).addClass('sc_button_hover_'+PJ_STORAGE['button_hover']);
		if (PJ_STORAGE['button_hover']!='arrow') {
			jQuery('input[type="submit"]:not([class*="sc_button_hover_"]),\
					...
					.socials_wrap:not(.socials_type_drop) .social_icons:not([class*="sc_button_hover_"]),\
...
jQuery(document).on('action.init_shortcodes', function(e, container) {
	...
	if (container.find('.sc_form_form:not(.inited):not(.contact_1)').length > 0) {
		container.find('.sc_form_form:not(.inited):not(.contact_1)')
			.addClass('inited')
			.submit(function(e) {
				"use strict";
